...
def convert_checkpoint(input_file, output_file, arguments):
    ...
    if 'extras' not in checkpoint:
        logging.warning("No `extras` in checkpoint file.")
        checkpoint['extras'] = {}
...
def main():
    ...
    if args.clip_mode == 'SCALE' and not args.scale:
        logging.warning(f'Using the default scale factor of {DEFAULT_SCALE:.2f}.\n')
        args.scale = DEFAULT_SCALE
    if args.clip_mode == 'STDDEV' and not args.stddev:
        logging.warning(f'Using the default number of standard deviations of {DEFAULT_STDDEV:.2f}.\n')
