...
from cfsai_backend_izer.izer import (apbaccess, assets, compute, console, datamem, kbias, kdedup, kernels, latency,
                  load, op, rtlsim, state, stats)
from cfsai_backend_izer.izer import tornadocnn as tc
from cfsai_backend_izer.izer.eprint import eprint, nprint, wprint
from cfsai_backend_izer.izer.names import layer_pfx, layer_str
from cfsai_backend_izer.izer.simulate import (conv1d_layer, conv2d_layer, convtranspose2d_layer, eltwise_layer,
                           passthrough_layer, pooling_layer, print_data, show_data)
from cfsai_backend_izer.izer.utils import ffs, fls, overlap, plural, popcount
...
ASSET_PREFIX = Path(__file__).parent
...
class Backend(backend.Backend):
    ...
    def create_net(self) -> str:  # pylint: disable=too-many-locals,too-many-branches
        ...
        if not os.path.isdir(str(ASSET_PREFIX / 'assets')):
            raise IzerError('The assets folder is missing from the current directory.')
        ...
        if start_layer > 0 and not tc.dev.SUPPORT_LINK_LAYER:
            raise IzerError('`--start-layer` is not supported on this device.')

        if start_layer > tc.dev.MAX_START_LAYER:
            raise IzerError(f'`--start-layer` is set to {start_layer}, but the device only supports '
                   f'a maximum of {tc.dev.MAX_START_LAYER}.')

        if link_layer and not tc.dev.SUPPORT_LINK_LAYER:
            raise IzerError('`--link-layer` is not supported on this device.')

        if any(rd_ahead) and not tc.dev.SUPPORT_READ_AHEAD:
            raise IzerError('`readahead` is not supported on this device.')

        if any(calcx4) and not tc.dev.SUPPORT_CALCX4:
            raise IzerError('`calcx4` is not supported on this device.')

        if state.pipeline and not tc.dev.SUPPORT_PIPELINE:
            raise IzerError('`--pipeline` is not supported on this device.')

        if state.pll and not tc.dev.SUPPORT_PLL:
            raise IzerError('`--pll` is not supported on this device.')

        if state.fifo_go and not tc.dev.SUPPORT_FIFO_GO:
            raise IzerError('`--fifo-go` is not supported on this device.')

        if snoop is not None and not tc.dev.SUPPORT_SNOOP:
            raise IzerError('`snoop` is not supported on this device.')

        if oneshot and not tc.dev.SUPPORT_ONESHOT:
            raise IzerError('`--one-shot` is not supported on this device.')
        ...
        if not state.balance_power and not state.pll:
            raise IzerError('`--max-speed` requires `--pll` or `--pipeline`.')
        ...
        if clock_speed // state.clock_divider > tc.dev.MAX_NO_PIPELINE_SPEED and not pipeline:
            logger.warning(f'For a CNN clock speed of {clock_speed} MHz, the pipeline must be enabled.')
        elif clock_speed // state.clock_divider <= tc.dev.MAX_NO_PIPELINE_SPEED and pipeline:
            logger.debug(f'For a CNN clock speed of {clock_speed} MHz, the pipeline can be disabled.')
        if state.clock_divider > tc.dev.MAX_CNNCLKDIV:
            logger.debug(f'The clock divider of {state.clock_divider} exceeds the device maximum '
        ...
        if (state.rtl_preload or state.rtl_preload_weights or state.result_output) \
           and not tc.dev.SUPPORT_SIM_PRELOAD:
            raise IzerError('`--rtl-preload` and `--result-output` are not supported on this device.')

        if embedded_code and any(calcx4) and not state.new_kernel_loader:
            logger.warning('Enabling --new-kernel-loader since calcx4 is used.')
            state.new_kernel_loader = True
            state.compact_weights = False

        if not state.new_kernel_loader and state.mexpress:
            if any(calcx4):
                logger.warning('Ignoring --mexpress since calcx4 is used.')
        ...
        if not fifo and state.synthesize_input is not None:
            raise IzerError('`--synthesize-input` requires `--fifo`')
        if big_data[start_layer] and state.synthesize_input is not None:
            raise IzerError('`--synthesize-input` requires `data_format: HWC`')

        if fifo:
            if start_layer != 0:
                raise IzerError('`--start_layer` must be 0 when using a FIFO.')

            if input_chan[start_layer] > 16 \
               or big_data[start_layer] and input_chan[start_layer] > 4:
                raise IzerError('Using the FIFO is restricted to a maximum of 4 input channels (CHW) or '
            ...
            if big_data[start_layer] and processor_map[start_layer] & ~0x0001000100010001 != 0 \
               ...
               and processor_map[start_layer] & ~0x000f000f000f000f != 0:
                raise IzerError('The FIFO is restricted to processors 0, 16, 32, 48 (CHW) or '
                       '0-3, 16-19, 32-35, 48-51 (HWC).')
            if fast_fifo:
                if big_data[start_layer] and input_chan[start_layer] > 1:
                    raise IzerError('Fast FIFO supports only a single CHW input channel; '
                           f'this test is using {input_chan[start_layer]} channels.')
                elif not big_data[start_layer] and input_chan[start_layer] > 4:
                    raise IzerError('Fast FIFO supports up to four HWC input channels; '
                           f'this test is using {input_chan[start_layer]} channels.')
                if processor_map[start_layer] != 1 and processor_map[start_layer] & 0x0e == 0:
                    fifo_group = False
                if output_width[start_layer] != 8:
                    raise IzerError('Single-layer fast FIFO setup requires output width of 8.')
                if operator[start_layer] == op.NONE:
                    raise IzerError('Fast FIFO requires a convolution operation in the first layer.')
        elif streaming[start_layer] and not allow_streaming:
            raise IzerError('Streaming in the first layer requires use of a FIFO.')
        if any(streaming) and start_layer != 0:
            raise IzerError('`--start_layer` must be 0 when using streaming.')

        for ll in range(min(tc.dev.MAX_STREAM_LAYERS + 1, layers)):
            if next_sequence[ll] != -1 and next_sequence[ll] != ll + 1 and streaming[ll]:
                raise IzerError(f'{layer_pfx(ll)}`next_sequence` must be {layer_str(ll+1)} when '
            ...
            if tc.dev.EMULATE_1X1_STREAMING and kernel_size[ll] == [1, 1] \
               ...
               and (streaming[ll] or prev_sequence[ll] >= 0 and streaming[prev_sequence[ll]]):
                logger.debug(f'{layer_pfx(ll)}Using 3x3 kernel hardware for layer with 1x1 kernel due '
                       'to streaming.')
            ...
            if not tc.dev.SUPPORT_STREAM_NONPAD_FINAL and streaming[ll] \
               ...
               and (padding[ll][0] == 0 or padding[ll][1] == 0):
                raise IzerError(f'{layer_pfx(ll)}Padding for the final streaming layer must not '
                       'be zero.')

            if not tc.dev.SUPPORT_STREAMING_PASSTHROUGH \
               and operator[ll] == op.NONE and streaming[ll]:
                raise IzerError(f'{layer_pfx(ll)}Passthrough operations are not supported for streaming '
                       'layers.')

        mlator = state.mlator

        if state.softmax and output_width[terminating_layer] == 8:
            logger.warning('--softmax should only be used with `output_width: 32`.')

        if fast_fifo and not riscv:
            raise IzerError('--fast-fifo requires --riscv')

        if state.sleep and not riscv:
            raise IzerError('--deepsleep requires --riscv')

        if oneshot and timer is not None:
            raise IzerError('--timer is not supported when using --one-shot')

        if not tc.dev.SUPPORT_KERNEL_BYPASS \
           and any(bypass[ll] for ll in range(first_layer_used, layers)):
            raise IzerError('Kernel bypass is not supported on this device.')
        ...
        if binary_quantization and not tc.dev.SUPPORT_BINARY_WEIGHTS:
            raise IzerError('Binary weights (-1/+1) are not supported on this device.')
        ...
        if repeat_layers * (final_layer + sum_hw_layers) > tc.dev.MAX_LAYERS:
            ...
            raise IzerError(f'{rep}The adjusted layer count ({final_layer + sum_hw_layers}) '
        ...
        for ll in range(first_layer_used, layers):
            if quantization[ll] == 1 and binary_quantization:
                raise IzerError(f'{layer_pfx(ll)}Cannot combine binary quantization with '
                       '1-bit quantization.')
            ...
            if output_shift[ll] < -15 or output_shift[ll] > 15:
                ...
                raise IzerError(f'{layer_pfx(ll)}{abs(quantization[ll])}-bit weight '
            ...
            if big_data[ll]:
                ...
                while p:
                    if popcount(p & (tc.dev.P_SHARED-1)) > 1:
                        raise IzerError(f'{layer_pfx(ll)}CHW input format, but multiple '
            ...
            if not streaming[ll] and in_size + in_offset[ll] > tc.dev.INSTANCE_WIDTH*16:
                raise IzerError(f'{layer_pfx(ll)}{1 if big_data[ll] else 4} '
            ...
            if operator[ll] != op.CONV1D:
                ...
                if operands[ll] > 1:
                    raise IzerError(f'{layer_pfx(ll)}Element-wise operations cannot be '
                           'combined with Conv1d.')

                if not tc.dev.SUPPORT_MULTIPASS_PADDED_CONV1D and padding[ll][0] > 0 \
                   and in_expand[ll] > 1:
                    raise IzerError(f'{layer_pfx(ll)}This device does not support padded Conv1d '
            ...
            if operator[ll] == op.NONE and next_sequence[ll] != -1 \
               ...
               and not any(ll in c if c is not None else False for c in in_sequences):
                logger.debug(f'{layer_pfx(ll)}Passthrough layer is followed by passthrough '
                ...
                if operands[ll] > 0 and pool[ll][0] == 1 and pool[ll][1] == 1 \
                   ...
                   and operands[next_sequence[ll]] == 1:
                    logger.debug('Use `pool_first: False` to combine element-wise and pooling layers '
            ...
            if not pool_first[ll] and operands[ll] > tc.dev.MAX_POOL_LAST_ELEMENTS \
               and (pool[ll][0] > 1 or pool[ll][1] > 1):
                raise IzerError(f'"pool last" supports a maximum of {tc.dev.MAX_POOL_LAST_ELEMENTS} '
                       'element-wise operands on this device.')

            if dilation[ll][0] > 1:
                if operator[ll] != op.CONV1D:
                    raise IzerError(f'{layer_pfx(ll)}`dilation` > 1 is supported for Conv1d only.')

                if kernel_size[ll][0] == 1:
                    raise IzerError(f'{layer_pfx(ll)}Kernel length must be greater than 1 to use '
                           '`dilation` > 1.')
                ...
                if (kernel_size[ll][0] - 1) * dilation[ll][0] < 9:
                    ...
                    # Use Conv2d
                    if pool[ll][0] != 1:
                        raise IzerError(f'{layer_pfx(ll)}Pooling must be 1 to use `dilation` > 4.')
                    if padding[ll][0] > tc.dev.MAX_DILATION_1D_PAD:
                        raise IzerError(f'{layer_pfx(ll)}Padding must be '
                               f'{tc.dev.MAX_DILATION_1D_PAD} or smaller to use `dilation` > 4.')
                    if operands[ll] != 1:
                        raise IzerError(f'{layer_pfx(ll)}Operands must be 1 to use `dilation` > 4.')
                    if bypass[ll] or flatten[ll] or rd_ahead[ll] or streaming[ll]:
                        raise IzerError(f'{layer_pfx(ll)}`bypass`, `flatten`, `rd_ahead`, '
                               '`streaming` must be False to use `dilation` > 4.')
                    if dilation[ll][0] > tc.dev.MAX_DILATION_1D:
                        raise IzerError(f'{layer_pfx(ll)}`dilation` must be '
                               f'{tc.dev.MAX_DILATION_1D} or smaller for Conv1d operations.')

                    logger.debug(f'{layer_pfx(ll)}Using Conv2d hardware for dilated Conv1d.')
                    ...
                    if out_offset[ll] < out_ignore[ll]:
                        raise IzerError(f'{layer_pfx(ll)}`out_offset` used with dilation of '
                               f'{dilation[ll][0]} must be at least {out_ignore[ll]:04x}.')
                else:
                    raise IzerError(f'{layer_pfx(ll)}Kernel length must be '
            ...
            if (not streaming[ll] or ll == terminating_layer) \
               and out_size + out_offset[ll] > tc.dev.INSTANCE_WIDTH*16:
                raise IzerError(f'{layer_pfx(ll)}HWC (4 channels/word) '
            ...
            if hw_operator[ll] == op.NONE:
                if activation[ll] is not None:
                    raise IzerError(f'{layer_pfx(ll)}Pass-through layers must not use activation.')
                if padding[ll][0] != 0 or padding[ll][1] != 0:
                    raise IzerError(f'{layer_pfx(ll)}Padding must be zero for passthrough layers.')
                if output_shift[ll] != 0 and output_shift[ll] is not None:
                    raise IzerError(f'{layer_pfx(ll)}`output_shift` must be zero for passthrough '
                           'layers.')
                if (pool[ll][0] > 1 or pool[ll][1] > 1) \
                   and in_expand[ll] > tc.dev.MAX_POOL_PASSES \
                   and (hw_pooled_dim[ll][0] > 1 or hw_pooled_dim[ll][1] > 1):
                    raise IzerError(f'{layer_pfx(ll)}pooling in passthrough layer uses '
                ...
                if hw_operator[ll] == op.CONVTRANSPOSE2D:
                    ...
                    if hw_padding[ll][0] not in tc.dev.SUPPORTED_X2D_PADS \
                       or hw_padding[ll][1] not in tc.dev.SUPPORTED_X2D_PADS:
                        raise IzerError(f'{layer_pfx(ll)}The selected padding ({padding[ll]}) for '
                    ...
                    if output_padding[ll][0] not in tc.dev.SUPPORTED_X2D_OUTPUT_PADS \
                       or output_padding[ll][1] not in tc.dev.SUPPORTED_X2D_OUTPUT_PADS:
                        raise IzerError(f'{layer_pfx(ll)}The selected output padding '
            ...
            if hw_operator[ll] != op.CONVTRANSPOSE2D and (output_padding[ll][0] != 0
                ...
                raise IzerError(f'{layer_pfx(ll)}Output padding must be 0 for this operator.')

            if input_chan[ll] % conv_groups[ll] != 0 or output_chan[ll] % conv_groups[ll] != 0:
                raise IzerError(f'{layer_pfx(ll)}convolution groups ({conv_groups[ll]}) does not '
                       f'divide the input channels ({input_chan[ll]}) or '
                       f'output channels ({output_chan[ll]}).')

            if flatten[ll] and hw_operator[ll] == op.NONE:
                raise IzerError(f'{layer_pfx(ll)}`flatten` is not compatible with passthrough '
                       'layers.')

            if flatten[ll] and (pool[ll][0] > 1 or pool[ll][1] > 1):
                raise IzerError(f'{layer_pfx(ll)}`flatten` is not compatible with pooling.')

            if flatten[ll] and streaming[ll]:
                raise IzerError(f'{layer_pfx(ll)}`flatten` is not compatible with streaming.')

            if conv_groups[ll] > 1:
                if not tc.dev.SUPPORT_DEPTHWISE:
                    raise IzerError(f'{layer_pfx(ll)}Convolution groups ({conv_groups[ll]}) > 1 are not '
                           f' supported on this device.')
                if conv_groups[ll] != input_chan[ll] or conv_groups[ll] != output_chan[ll]:
                    raise IzerError(f'{layer_pfx(ll)}Convolution groups ({conv_groups[ll]}) must be equal '
                ...
                if bias_group_map[ll] is not None:
                    raise IzerError(f'{layer_pfx(ll)}`bias_group` is not supported for depth-wise layers.')
            ...
            if input_skip[ll] != 0 and not tc.dev.SUPPORT_MULTIPASS_STRIDE:
                raise IzerError(f'{layer_pfx(ll)}`read_gap` must be 0 for this device.')
            if input_skip[ll] != 0 and in_expand[ll] > 1 \
               and not tc.dev.SUPPORT_MULTIPASS_READ_STRIDE:
                raise IzerError(f'{layer_pfx(ll)}`read_gap` must be 0 when using more than 64 channels or '
            ...
            if pool_dilation[ll][0] < 1 or pool_dilation[ll][1] < 1 \
               ...
               or pool_dilation[ll][1] > tc.dev.MAX_POOL_DILATION:
                raise IzerError(f'{layer_pfx(ll)}`pool_dilation` values must be 1 or greater, and '
            ...
            if in_sequences[ll] is not None:
                ...
                if or_map != processor_map[ll]:
                    logger.warning(f'{layer_pfx(ll)}The combination of the `in_sequences` '
                           '`output_processor_map`s does not combine to `processor_map`.')
                if and_map != processor_map[ll]:
                    # Concatenate channels sequentially
                    if and_map != 0:
                        logger.warning(f'{layer_pfx(ll)}Channel concatenation has `output_processor_map` '
                               f'overlap for `in_sequences` {in_sequences[ll]}.')
                    for i, lt in enumerate(in_sequences[ll]):
                        if lt != -1 and in_offset[ll] != out_offset[lt]:
                            logger.warning(f'{layer_pfx(ll)}`in_offset` (0x{in_offset[ll]:04x}, for '
                ...
                if operands[ll] == 1:  # cat
                    if write_gap[ll] == 0:
                        ...
                        for lt in in_sequences[ll]:
                            ...
                            if first_proc < min_proc:
                                logger.warning(f'{layer_pfx(ll)}In `in_sequences` {in_sequences[ll]}, '
                    ...
                    for lt in in_sequences[ll]:
                        ...
                        if eltwise_proc_map not in (0, emap):
                            raise IzerError(f'{layer_pfx(ll)}In `in_sequences` {in_sequences[ll]}, '
                ...
                if processor_map[ll] & emap != processor_map[ll]:
                    logger.warning(f'{layer_pfx(ll)}The processor map {processor_map[ll]:016x} specifies '
                           'processors that have no data from any of the input sequences '
                           f'{in_sequences[ll]}.')

            if output_width[ll] != 8 and hw_operator[ll] == op.NONE:
                raise IzerError(f'{layer_pfx(ll)}'
        ...
        try:
            ...
            if not overwrite:
                raise IzerError(f'The target folder {target_dir}, exists. Use --overwrite to proceed.')
            else:
                logger.debug(f'--overwrite specified, writing to, {target_dir}, even though it exists.')
        ...
        for ll in range(first_layer_used, layers):
            ...
            if input_chan[ll] > tc.dev.MAX_CHANNELS:
                raise IzerError(f'{layer_pfx(ll)}Configured for {input_chan[ll]} input channels{fl}, which '
                       f'exceeds the system maximum of {tc.dev.MAX_CHANNELS}.')
            if output_chan[ll] > tc.dev.MAX_CHANNELS:
                raise IzerError(f'{layer_pfx(ll)}Configured for {output_chan[ll]} output channels, which '
                       f'exceeds the system maximum of {tc.dev.MAX_CHANNELS}.')
            if (ll != start_layer or not fast_fifo_quad) \
               and popcount(processor_map[ll]) != in_expand_thresh[ll]:
                raise IzerError(f'{layer_pfx(ll)}{input_chan[ll]} input '
            ...
            if popcount(output_processor_map[ll]) != out_expand_thresh[ll]:
                raise IzerError(f'{layer_pfx(ll)}{output_chan[ll]} output '
            ...
            if hw_operator[ll] == op.NONE:
                for group in range(tc.dev.P_NUMGROUPS):
                    ...
                    if out_pro != 0:
                        out_pro >>= ffs(out_pro)
                    if out_pro != in_pro:
                        raise IzerError(f'{layer_pfx(ll)}The output processors for a pass-through layer '
            ...
            if conv_groups[ll] > 1:
                if ffs(output_processor_map[ll]) % tc.dev.P_SHARED != 0:
                    raise IzerError(f'{layer_pfx(ll)}Output processors for depth-wise convolutions '
                ...
                if ffs(processor_map[ll]) % tc.dev.P_SHARED != 0 \
                   and (processor_map[ll] >> ffs(processor_map[ll])) // 2**tc.dev.P_NUMPRO > 0:
                    raise IzerError(f'{layer_pfx(ll)}When spanning groups for depth-wise convolutions, '
                ...
                if processor_map[ll] == output_processor_map[ll]:
                    ...
                    logger.debug(f'{layer_pfx(ll)}depth-wise convolution moves data across processors. '
               ...
               and operands[ll] * in_expand[ll] != operands[ll] + in_expand[ll]:
                if hw_operator[ll] != op.NONE or pool[ll][0] > 1 or pool[ll][1] > 1 \
                   or pool_stride[ll][0] > 1 or pool_stride[ll][1] > 1:
                    raise IzerError(f'{layer_pfx(ll)}The element-wise operation exceeds a multi-pass of 2 '
            ...
            if ll != start_layer and input_chan[ll] % 4 != 0 and state.energy_warning:
                logger.debug(f'{layer_pfx(ll)}The input channel count ({input_chan[ll]}) is not '
        ...
        if 0 not in groups_used:
            raise IzerError('Quadrant 0 is not used, this is currently unsupported.')

        for ll in range(first_layer_used, layers):
            if bias_group_map[ll] is not None:
                for e in bias_group_map[ll]:
                    if e not in groups_used:
                        raise IzerError(f'{layer_pfx(ll)}`bias_quadrant` references unused quadrant {e}. '
            ...
            for r in range(repeat_layers):
                for ll in range(first_layer_used, layers):
                    ...
                    for gindex, group in enumerate(groups_used):
                                   ...
                                   and operands[lt] == 1:
                                    ...
                                    if in_offset[lt] != ll_offset \
                                       and out_offset[ll] != offs * 4:
                                        logger.warning(f'{layer_pfx(ll)}The input offset '
                                    ...
                                    if (
                                        ...
                                        logger.warning(f'{layer_pfx(ll)}The input dimensions of the next '
                        ...
                        if in_exp >= 2**4:
                            raise IzerError(f'{layer_pfx(ll)}Input expansion of {in_exp+1} exceeds device '
                                   f'limit of {2**4}.')
                        ...
                        if not tc.dev.SUPPORT_MULTIPASS_ELTWISE_CONV_BIAS \
                           ...
                           and operands[ll] > 1:
                            raise IzerError(f'{layer_pfx(ll)}On this device, multi-pass element-wise '
                        ...
                        if calcx4[ll]:
                            ...
                            if not tc.dev.SUPPORT_MULTIPASS_X4_PARTIALQUAD \
                               and out_expand[ll] > 1 and tc.dev.MAX_PROC != 64:
                                raise IzerError(f'{layer_pfx(ll)}This device does not support `calcx4` '
                        ...
                        if delta2 >= 2**tc.dev.MAX_DSVAL2_BITS:
                            raise IzerError(f'Layer {ll}: delta2 ({delta2}) exceeds device maximum '
                        ...
                        if rd_ahead[ll]:
                            ...
                            if in_instance[0] == out_instance[0] \
                               or in_instance[1] == out_instance[1]:
                                raise IzerError(f'{layer_pfx(ll)}Input and output cannot use the same data '
        ...
        while ll < layers:
                ...
                if len(in_sequences[ll]) > 1:
                    ...
                    if err_concat is not None:
                        try:
                            ...
                            raise IzerError(f'{layer_pfx(ll)}Input data concatenation unsuccessful: ',
                                   err_concat, err)
            ...
            if operator[ll] == op.CONV1D:
                if out_size[0] != in_chan \
                   or out_size[1] != pooled_dim[ll][0] or pooled_dim[ll][1] != 1:
                    raise IzerError(f'{layer_pfx(ll)}Input dimensions do not match. '
            ...
            if operator[ll] in [op.CONV2D, op.LINEAR]:
                ...
                raise IzerError(f'Unknown operator `{op.string(operator[ll])}`.')
            ...
            if buffer_insert[ll] is not None:
                try:
                    ...
                    raise IzerError(f'{layer_pfx(ll)}Buffer insertion unsuccessful. Check that the '
            ...
            if not np.any(out_buf) and state.warn_zero:
                logger.warning(f'{layer_pfx(ll)}All output values for the given sample input are '
        ...
        if not embedded_code and not block_mode:
            ...
            s.copy(str(ASSET_PREFIX / 'assets'), 'rtlsim-ai' + str(device), base_directory, test_name)
            if riscv_cache:
                assets.copy(str(ASSET_PREFIX / 'assets'), 'rtlsim-riscv-cache-ai' + str(device), base_directory,
                            test_name)
            elif riscv_flash:
                assets.copy(str(ASSET_PREFIX / 'assets'), 'rtlsim-riscv-flash-ai' + str(device), base_directory,
                            test_name)
            elif riscv:
                assets.copy(str(ASSET_PREFIX / 'assets'), 'rtlsim-riscv-ai' + str(device), base_directory, test_name)
            if result_output:
                assets.copy(str(ASSET_PREFIX / 'assets'), 'rtlsim-verify-output', base_directory, test_name)
        elif block_mode:
            assets.copy(str(ASSET_PREFIX / 'assets'), 'blocklevel-ai' + str(device), base_directory, test_name)
        elif embedded_code:
            output_count = 0
            ...
            #if riscv:
            #    assets.from_template(str(ASSET_PREFIX / 'assets'), 'embedded-riscv-ai' + str(device), base_directory,
            #                         test_name, board_name)
            #else:
            #    assets.from_template(str(ASSET_PREFIX / 'assets'), 'embedded-ai' + str(device), base_directory,
            #                         test_name, board_name)
            ...
            #assets.from_template(str(ASSET_PREFIX / 'assets'), 'eclipse', base_directory, test_name, board_name)
            ...
            assets.from_template(str(ASSET_PREFIX / 'assets'), 'device-all', base_directory,
                                 test_name, board_name, insert=insert)
            assets.from_template(str(ASSET_PREFIX / 'assets'), 'device-ai' + str(device), base_directory,
                                 test_name, board_name)

        logger.debug(stats.summary(factor=repeat_layers, group_bias_max=group_bias_max))
