...
def unload(
    ...
    if not scale_output and final_scale_detected:
        logging.warning('Non-zero output scale detected, but --scale-output not set. '
    ...
    if scale_output and not final_scale_detected:
        logging.debug('--scale-output set, but all output scales are zero. '
               'Unload operation will be realized without scaling.')
    ...
    for ll, e in enumerate(output_layer):
        ...
        if output_width[ll] != 8:
            if mlator and mlator_warning:
                logging.warning(f'{lname}Ignoring --mlator for 32-bit output.')
        elif input_shape[ll][0] > 1 and input_shape[ll][1] * input_shape[ll][2] % 4 != 0:
            if mlator and mlator_warning:
                logging.warning(f'{lname}Ignoring --mlator for '
            ...
            if mlator and mlator_warning:
                logging.warning(f'{lname}--mlator should only be used with 4 or more 8-bit outputs '
                       ' per channel; ignoring.')
        elif mlator:
            mlator_layers.append(ll)
        elif state.embedded_code and mlator_warning:
            logging.debug(f'{lname}Use --mlator to optimize cnn_unload() for 8-bit output values.')
...
def verify(
    ...
    if mlator and output_width != 8:
        logging.warning(f'{layer_pfx(ll)}Ignoring --mlator for 32-bit output.')
        mlator = False
