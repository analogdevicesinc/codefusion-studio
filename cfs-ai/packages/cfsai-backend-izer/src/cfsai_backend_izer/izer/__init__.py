...
from .args_dataclass import CNNGeneratorArgs
...
def codegen(args: CNNGeneratorArgs):
    """
    Izer codegen
    """
    ...
    commandline.set_state(Namespace(**asdict(args)))
    ...
    if cfg['arch'] != 'test':
        if not args.checkpoint_file:
            raise IzerError('Model file passed to synthesis tool')
    ...
    if layers != cfg_layers:
        raise IzerError(
            f'Number of layers in the YAML configuration file ({cfg_layers}) '
    ...
    if any(p < 0 or p >= 4*tc.dev.MEM_SIZE for p in params['output_offset']):
        raise IzerError(
            'Unsupported value for `out_offset` in YAML configuration. Supported on this '
    ...
    if any(q != 8 for q in params['bias_quantization']):
        raise IzerError('All bias quantization configuration values must be 8.')
        #eprint('All bias quantization configuration values must be 8.')

    logger.debug(f'Configuring data set: {cfg["dataset"]}.')
    #print(f'Configuring data set: {cfg["dataset"]}.')
    if args.input_shape is not None:
        data = sampledata.get_random(
        ...
        if args.sample_input is None:
            sampledata_file = str(Path(__file__).parent.joinpath(
                'sample_inputs', 
                'npy',
                f'sample_{cfg["dataset"].lower()}.npy'
            ))
        else:
    ...
    if np.max(data) > 127 or np.min(data) < -128:
        raise IzerError(
            f'Input data {sampledata_file} contains values that are outside the limits of '
    ...
    while ll < layers:
        if in_sequences[ll] is not None:
            ...
            for e in in_sequences[ll]:
                if chan != output_channels[e]:
                    ...
                    raise IzerError(
                        f'{layer_pfx(ll)}`in_sequences` [{l_str}] for the element-wise '
            ...
            if operands[ll] > 1:
                if gap != operands[ll] - 1:
                    logger.warning(
                        f'{layer_pfx(ll)}The element-wise operation has {operands[ll]} '
                ...
                if input_skip[ll] != gap // operands[ll]:
                    logger.warning(
                        f'{layer_pfx(ll)}`read_gap` {input_skip[ll]} does not match the '
                ...
                if gap != l_inseq - 1:
                    logger.warning(
                        f'{layer_pfx(ll)}The channel interleave operation has {l_inseq} '
        ...
        if next_sequence[ll] != -1 and next_sequence[ll] != ll + 1 \
           and not tc.dev.SUPPORT_LINK_LAYER:
            raise IzerError(f'{layer_pfx(ll)}`next_sequence` is not supported on this device.')
            #eprint(f'{layer_pfx(ll)}`next_sequence` is not supported on this device.')
        elif next_sequence[ll] > layers:
            logger.warning(f'{layer_pfx(ll)}`next_sequence` exceeds available layers, '
                   'setting to `stop`.')
    ...
    if not any(output_layer):
        ...
        logging.warning(f'The final layer {layer_str(ll)} is not designated as an `output` layer.')
    ...
    if not tc.dev.SUPPORT_ARBITRARY_OUTPUT_WIDTH:
        ...
        if output_width[final_layer] != 8 and activation[final_layer] is not None:
            raise IzerError(f'{layer_pfx(ll)}`output_width` must be 8 when activation is used.')
            #eprint(f'{layer_pfx(ll)}`output_width` must be 8 when activation is used.')

    if ll == 0 and (pool[0][0] != 1 or pool[0][1] != 1):
        logging.warning(f'{layer_pfx(0)}The input layer uses pooling. Consider downscaling the input data '
    ...
    while ll < layers:
        if input_channels[ll] <= 0:
            raise IzerError(f'{layer_pfx(ll)}`in_channels` is required.')
        ...
        if ll != args.start_layer:
            ...
            if input_offset[ll] is None:
                ...
                logging.warning(
                    f'{layer_pfx(ll)}Non-default `in_offset: 0x{input_offset[ll]:04x}`'
            ...
            if streaming[ll] and not streaming[prev_sequence[ll]]:
                raise IzerError(
                    f'Layer {layer_str(ll)} is a streaming layer, but the previous layer '
            ...
            if big_data[ll]:
                raise IzerError(
                    f'{layer_pfx(ll)}`data_format` CHW can only be configured for the '
                       'first layer.'
                )
        ...
        if output_width[ll] != 8 and next_seq != -1 \
           and (in_sequences[next_seq] is None or ll in in_sequences[next_seq]):
            logging.warning(f'{layer_pfx(ll)}`output_width` should be 8 for intermediate layers.')
        ...
        if in_sequences[ll] is not None:
            if tc.dev.SUPPORT_LINK_LAYER:
                if any(i > len(in_sequences) for i in in_sequences[ll]):
                    raise IzerError(f'{layer_pfx(ll)}`in_sequences` cannot be greater than the '
                           'last layer.')
                ...
                if any(i >= ll for i in in_sequences[ll]):
                    raise IzerError(
                        f'{layer_pfx(ll)}`in_sequences` cannot be greater than layer '
        ...
        if input_dim[ll] is None:
            ...
            if in_sequences[ll] is not None:
                ...
                for e in in_sequences[ll]:
                    ...
                    if odim != dim and conf_input_dim[ll] is None:
                        raise IzerError(
                            f'{layer_pfx(ll)}Cannot concatenate outputs of different '
            ...
            if conf_input_dim[ll] is None:
                ...
                if input_dim[ll] is None:
                    raise IzerError(
                        f'{layer_pfx(ll)}No input dimension information. Use '
                ...
                if input_dim[ll][1] == 1 and operator[ll] in [op.CONV2D, op.CONVTRANSPOSE2D] \
                   and prev_op == op.CONV1D:
                    logging.warning(
                        f'{layer_pfx(ll)}Using 1-dimensional data {input_dim[ll][0]}x'
                ...
                if pixels > 0 and pixelcount != pixels:
                    raise IzerError(
                        f'{layer_pfx(ll)}The configured `in_dim` (pixel count '
        ...
        if operator[ll] != op.CONV1D and (operator[ll] != op.NONE or input_dim[ll][1] != 1):
            if pool_stride[ll][0] != pool_stride[ll][1]:
                raise IzerError(
                    '{layer_pfx(ll)}{op.string(operator[ll])} does not support '
        ...
        if any(dim == 0 for dim in pooled_dim[ll]):
            raise IzerError(
                f'{layer_pfx(ll)}Pooling or zero-padding results in a zero data '
        ...
        if operator[ll] != op.CONV1D and (operator[ll] != op.NONE or input_dim[ll][1] != 1):
            if stride[ll][0] != stride[ll][1]:
                raise IzerError(
                    f'{layer_pfx(ll)}{op.string(operator[ll])} does not support '
            ...
            if operator[ll] != op.CONVTRANSPOSE2D and stride[ll][0] != 1:
                raise IzerError(
                    f'{layer_pfx(ll)}{op.string(operator[ll])} does not support stride '
            ...
            if flatten[ll]:
                if pooled_dim[ll][0] * pooled_dim[ll][1] > 256:
                    raise IzerError(
                        f'{layer_pfx(ll)}`flatten` exceeds supported input dimensions '
                ...
                if pooled_dim[ll][0] * pooled_dim[ll][1] == 1:
                    logging.warning(
                        f'{layer_pfx(ll)}`flatten` is not needed since input '
                           'dimensions are 1x1.'
                    )
        ...
        if operator[ll] == op.NONE and (padding[ll][0] > 0 or padding[ll][1] > 0):
            raise IzerError(
                f'{layer_pfx(ll)}Padding ({padding[ll]}) can only be used with a convolution '
                   'operation.'
            )
        ...
        if padding[ll][0] >= kernel_size[ll][0] or padding[ll][1] >= kernel_size[ll][1]:
            raise IzerError(
                f'{layer_pfx(ll)}Pad size ({padding[ll]}) is greater than or equal to'
        ...
        if any(dim > tc.dev.MAX_ROW_COL for dim in input_dim[ll]):
            raise IzerError(
                f'{layer_pfx(ll)}Input dimension {input_dim[ll]} exceeds system '
        ...
        if any(dim > tc.dev.MAX_ROW_COL for dim in output_dim[ll]):
            raise IzerError(
                f'{layer_pfx(ll)}Output dimension {output_dim[ll]} exceeds system '
                   f'maximum of {tc.dev.MAX_ROW_COL}.'
            )
        ...
        if any(dim == 0 for dim in output_dim[ll]):
            raise IzerError(
                f'{layer_pfx(ll)}Output dimension {output_dim[ll]} is zero.'
            )
        ...
        if activation[ll] is not None and operator[ll] == op.NONE:
            raise IzerError(
                f'{layer_pfx(ll)}Activation {op.act_string(activation[ll])} cannot '
        ...
        if buffer_shift[ll] is not None and operator[ll] is not op.NONE:
            raise IzerError(
                f'{layer_pfx(ll)}Buffer shift can only be used in a passthrough layer.'
            )
    ...
    if args.riscv and not args.riscv_cache and args.embedded_code:
        raise IzerError(
            'Embedded code on RISC-V requires --riscv-cache.'
        )
    ...
    # Instantiate backend
    module = locate('cfsai_backend_izer.izer.backend.' + tc.dev.backend)
