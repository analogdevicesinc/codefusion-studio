...
def load(
    ...
    logger.debug(f'Reading {checkpoint_file} to configure network weights...')

    if 'state_dict' not in checkpoint:
        raise IzerError("No `state_dict` in checkpoint file.")
    if 'arch' not in checkpoint:
        logging.warning("No `arch` in checkpoint file.")
        checkpoint_arch = ''
    else:
        checkpoint_arch = checkpoint['arch']
        if arch and checkpoint_arch.lower() != arch.lower():
            raise IzerError(f"Network architecture of configuration file ({arch}) does not match "
                   f"network architecture of checkpoint file ({checkpoint_arch}).")
    if 'epoch' not in checkpoint:
        raise IzerError("No `epoch` in checkpoint file.")
    ...
    for k in checkpoint_state.keys():
        ...
        if parameter in ['weight']:
            ...
            if np.all(w == 0):
                logging.warning(f'All weights for `{k}` are zero.')

            if w.ndim == 1:
                if this_op == 'bn':
                    raise IzerError('The checkpoint file contains 1-dimensional weights for '
            ...
            if w.ndim == 2:  # MLP
                if kernel_size[seq][0] != 1 or kernel_size[seq][1] != 1:
                    raise IzerError(f'The `kernel_size` for the MLP layer {layer_str(seq)} should '
                ...
                if kernel_size[seq][0] != w.shape[2] or kernel_size[seq][1] != 1:
                    raise IzerError(f'The `kernel_size` for the 1D layer {layer_str(seq)} should '
                ...
                if kernel_size[seq][0] != w.shape[2] \
                   or kernel_size[seq][1] != w.shape[3]:
                    raise IzerError(f'The `kernel_size` for the 2D layer {layer_str(seq)} should '
            ...
            if bias_name in checkpoint_state and seq not in no_bias:
                ...
                if np.all(w == 0):
                    logging.warning(f'All bias values for `{bias_name}` are zero.')
