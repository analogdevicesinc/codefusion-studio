...
export default class Compat extends BaseCommand<typeof Compat> {
  static description = `Analyze model compatibility with target SoC and core.`;
  ...
  static flags = {
    ...
    'report-file': Flags.string({
      name: 'report-file',
      summary: 'Path to output JSON report file',
      required: false
    }),
    ...
    dataset: Flags.string({
      char: 'd',
      summary:
        'Path or URL to the dataset file to use for compatibility analysis',
      required: false
    })
  };
  ...
  async run(): CliRunResponse {
    ...
    if (!flags.soc || !flags.core || !flags.model) {
      throw new RecoverableError(
        ...
        {
          suggestion:
            'Provide all three required flags to check model compatibility',
          example:
            'cfsutil ai compat --core CM4 --soc MAX32690 --model <model>'
        }
      );
    }
    ...
    const output = await aiPlugin.runCompat(ai, flags.model, {
      reportFile: flags['report-file'],
