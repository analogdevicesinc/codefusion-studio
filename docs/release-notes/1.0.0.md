...
CodeFusion Studio source can be found on {git-codefusion-studio}`GitHub <releases/tag/V1.0.0+>` under tag `V1.0.0`
...
- Support for Zephyr 3.7. [Get started](https://docs.zephyrproject.org/3.7.0/)
...
- M4 core breakpoints are also set on RISC-V core.
    - If debugging a dual-core application and setting a breakpoint on the M4 that could also apply to the RISC-V core such as a file and line combination or a symbol that is present in both images, then that breakpoint will also be applied to the RISC-V core erroneously.
