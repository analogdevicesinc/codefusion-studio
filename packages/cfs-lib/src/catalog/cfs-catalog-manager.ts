...
export class CatalogManager {
	private readonly apiClient: CfsApiClient | undefined;
	 ...
	 *
	 * @param {Object} params - The parameters for the CatalogManager.
	 * @param {string} params.catalogStoreDir - The directory where the catalog store is located.
	 * @param {string} [params.catalogBackupZipFile] - Path to the zip file with a backup copy of the catalog.
	 * @param {CfsApiClient} [params.apiClient] - API client to use (works offline if not provided).
	 */
	constructor({
		catalogStoreDir,
		...
		catalogBackupZipFile?: string;
		apiClient?: CfsApiClient;
	}) {
		this.catalogStoreDir = catalogStoreDir;
		this.catalogBackupZipFile = catalogBackupZipFile;
		this.apiClient = apiClient;

		// Ensure the catalog store directory exists
		fs.mkdirSync(this.catalogStoreDir, {
			recursive: true
		});
	...
	private async refreshCatalog(): Promise<void> {
		try {
			...
			if (err instanceof CatalogError) {
				...
				if (
					err.type !== "SERVICE_ERROR" ||
					(err.cause instanceof Error &&
						err.cause.message.includes("Unauthorized"))
				) {
					throw err;
				}
			}
