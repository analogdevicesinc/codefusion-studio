...
export default class CfsAuthLogin extends Command {
  static description = 'Login with a myAnalog account.';
  static flags = {
  ...
  async run(): Promise<void> {
    const {flags} = await this.parse(CfsAuthLogin);
    let session;
    let sessionManager;
    try {
    ...
    if (session) {
      this.log(`You are already logged in as ${session.userEmail}.`);
      this.log(
    ...
    try {
      ...
      this.error(
        new Error('An error occurred while trying to login.', {
          cause: error
        })
      );
    }
