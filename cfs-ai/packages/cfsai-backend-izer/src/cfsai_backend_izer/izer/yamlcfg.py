...
def parse(
    ...
    logging.debug(f'Reading {config_file} to configure network...')

    if not os.path.exists(config_file):
        raise IzerError(
            f'YAML configuration file {config_file} does not exist!'
        )
    ...
    if bool(cfg_set):
        raise IzerError(
            f'Configuration file {config_file} contains unknown key(s): {cfg_set}.'
        )
    ...
    if 'layers' not in cfg or 'arch' not in cfg or 'dataset' not in cfg:
        raise IzerError(
            f'Configuration file {config_file} does not contain '
    ...
    for ll in cfg['layers']:
        ...
        if 'out_offset' in ll:
            ...
            logging.warning(
                'Defaulting to `out_offset = 0` for '
                   f'layer sequence {sequence} in YAML configuration.'
            )
            #logging.warning('Defaulting to `out_offset = 0` for '
            #       f'layer sequence {sequence} in YAML configuration.')
    ...
    if 'data_buffer' in cfg:
        for ll in cfg['data_buffer']:
            ...
            if bool(cfg_set - set(['processors', 'dim', 'offset', 'width', 'channels', 'name'])):
                raise IzerError(
                    f'Configuration file {config_file} contains unknown key(s) for '
                       '`data_buffer`.'
                )
            ...
            if 'processors' not in cfg_set or 'dim' not in cfg_set or 'channels' not in cfg_set \
               or 'offset' not in cfg_set or 'name' not in cfg_set:
                raise IzerError(
                    f'`data_buffer` sequence in configuration file {config_file} does not '
    ...
    for ll, _ in enumerate(operator):
        ...
        if pool_stride[ll][0] is None:
            if pooling_enabled[ll]:
                logging.warning(f'Using default pool stride of 1 in layer {ll}.')
                #logging.warning(f'Using default pool stride of 1 in layer {ll}.')
            pool_stride[ll] = [1, 1]
    ...
    if 'unload' in cfg:
        for ll in cfg['unload']:
            ...
            if bool(cfg_set - set(['processors', 'dim', 'offset', 'width',
                                   'write_gap', 'channels'])):
                raise IzerError(
                    f'Configuration file {config_file} contains unknown key(s) for `unload`.'
                )
            ...
            if 'processors' not in cfg_set or 'dim' not in cfg_set or 'channels' not in cfg_set \
               or 'offset' not in cfg_set:
                raise IzerError(
                    f'`unload` sequence in configuration file {config_file} does not contain '
