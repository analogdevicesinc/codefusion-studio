...
def load(
    ...
    if not os.path.exists(fname):
        raise IzerError(f'Sample weights file {fname} does not exist!')
    with open(fname, mode='rb') as file:
        logger.debug(f'Reading weights from {fname}...')
        try:
            while True:
    ...
    while seq < cfg_layers:
        ...
        if current_quant > 8:  # Either way, more than 8 bits is an error
            raise IzerError('Model file includes values larger than 8 bit!')
        if quant == -1:
...
def load_bias(
    ...
    if cfg_bias is not None or filename is not None:
        ...
        if not os.path.exists(fname):
            raise IzerError(f'Sample bias file {fname} does not exist!')
        with open(fname, mode='rb') as file:
            logger.debug(f'Reading bias from {fname}...')
            try:
                while ll < layers:
