...
def load(
    ...
    if fixed_input and not embedded_code:
        raise IzerError('--fixed-input requires --embedded-code')
    ...
    if operands != data.shape[0] // input_size[0]:
        raise IzerError(f'Data input/weights mismatch: The sample data input has {data.shape[0]} channels '
    ...
    for ch in range(0, tc.dev.MAX_CHANNELS, step):
        ...
        if new_data_offs == data_offs:
            raise IzerError('Layer 0 processor map is misconfigured for data input. '
        ...
        if chw:
            ...
            if embedded_code and in_expand > 1:
                ...
                raise IzerError('--compact-data does not currently support multi-pass CHW input')
