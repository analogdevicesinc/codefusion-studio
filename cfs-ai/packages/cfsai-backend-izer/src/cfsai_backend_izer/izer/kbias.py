...
def load(
    ...
    for ll in range(start_layer, layers):
        ...
        if len(bias[ll]) != output_chan[ll]:
            raise IzerError(f'{layer_pfx(ll)}Output channel count {output_chan[ll]} does not match the '
                   f'number of bias values {len(bias[ll])}.')
        if not np.any(bias[ll] != 0):
            logging.warning(f'{layer_pfx(ll)}All bias values are zero. Ignoring the input.')
            continue
        ...
        def bias_add_byte(layer, group, val):
            ...
            if group_bias_max[group] >= tc.dev.BIAS_SIZE:
                raise IzerError(f'{layer_pfx(layer)}Bias memory capacity for group {group} exhausted, '
    ...
    for (ll, gmap, blen, _) in bias_map:
        ...
        if group_bias_max[group] + blen > tc.dev.BIAS_SIZE:
            raise IzerError(f'{layer_pfx(ll)}bias memory capacity exhausted - available groups: '
    ...
    for ll in range(start_layer, layers):
        ...
        if streaming[ll] and not tc.dev.SUPPORT_STREAM_BIAS:
            logging.warning(f'{layer_pfx(ll)}Combining streaming and a bias. '
