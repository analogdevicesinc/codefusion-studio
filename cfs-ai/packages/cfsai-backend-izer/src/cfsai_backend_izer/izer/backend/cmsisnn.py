...
from cfsai_backend_izer.izer import assets, op, state, stats, toplevel
from cfsai_backend_izer.izer import tornadocnn as tc
from cfsai_backend_izer.izer.eprint import eprint, wprint
...
class Backend(backend.Backend):
    ...
    def create_net(self) -> str:  # pylint: disable=too-many-locals,too-many-branches
        ...
        logger.warning('CMSIS-NN code generation is unsupported.')

        if output_width[-1] != 8 and operator[-1] != op.LINEAR:
            logger.warning('CMSIS-NN network generator does not currently support `output_width` that '
        ...
        for ll in range(layers):
            if quantization[ll] is None:
                ...
                logger.warning('CMSIS-NN network generator does not currently support `quantization` '
                       '!= 8. Forcing to 8 bit.')
            ...
            if input_chan[ll] % conv_groups[ll] != 0 or output_chan[ll] % conv_groups[ll] != 0:
                raise IzerError(f'Layer {ll}: convolution groups {conv_groups[ll]} does not divide '
        ...
        with open(
            ...
            for ll in range(layers):
                ...
                if in_sequences[ll] is not None:
                    if isinstance(in_sequences[ll], list):
                        try:
                            ...
                            raise IzerError('Error in input data concatenation layer:', err)
                    else:
                ...
                if operands[ll] > 1 and not pool_first[ll]:
                    raise IzerError("Element-wise operations are currently not implemented for CMSIS-NN")
                ...
                if operator[ll] != op.NONE:
                    ...
                    if operator[ll] in [op.CONVTRANSPOSE2D]:  # FIXME: Support ConvTranspose2d
                        raise IzerError("CMSIS-NN generator does not currently support the operator "
                    ...
                    if activation[ll] == op.ACT_RELU:
                        ...
                        raise IzerError("CMSIS-NN generator implements ReLU only.")
                    buffer0, buffer1 = buffer1, buffer0

                if not np.any(out_buf):
                    logger.warning(f'Layer {ll}: All output values for the given sample input are zero. '
